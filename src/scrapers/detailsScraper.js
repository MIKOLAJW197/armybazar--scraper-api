...
const offerDetailsScraper = (pageUrl) => {
  return axios(pageUrl)
    .then((response) => {
      ...
      const email = phone
        ? emailRegex.exec(phoneAndEmail[3])
        : emailRegex.exec(phoneAndEmail[2]);

      offerContact = new ContactDTO(
        $(contactJq).find("a").text(),
        phone,
        email,
        $(contactJq).prev().contents().last().text().trim()
      );

      return new ProductDetailsDTO(
        $("h2.outer.small").contents().last().text().substring(2),
        $("h1").text(),
